...
def run_cv(args):
    ...
    print("    #CHEM={:} | #DGNet={:} | #EXP={:} ".format(n_chem, n_dg, n_exp))
    ##################
    # normalize data
    #
    ##################
    train_df = train_df[chem_col_list+dg_col_list+exp_col_list+['resp']]
    valid_df = valid_df[chem_col_list+dg_col_list+exp_col_list+['resp']]
    test_df = test_df[chem_col_list+dg_col_list+exp_col_list+['resp']]
    ...
    chem_train_X_arr = train_df[chem_col_list].values
    dg_train_X_arr = train_df[dg_col_list].values
    exp_train_X_arr = train_df[exp_col_list].values
    
    chem_valid_X_arr = valid_df[chem_col_list].values
    dg_valid_X_arr = valid_df[dg_col_list].values
    exp_valid_X_arr = valid_df[exp_col_list].values

    chem_test_X_arr = test_df[chem_col_list].values
    dg_test_X_arr = test_df[dg_col_list].values
    exp_test_X_arr = test_df[exp_col_list].values
    print("BEFORE NORMALIZATION")
    print("chem={:} | dg={:} | exp={:}".format(
           chem_train_X_arr.shape, dg_train_X_arr.shape, exp_train_X_arr.shape))
    print("chem={:} | dg={:} | exp={:} ".format(
           chem_valid_X_arr.shape, dg_valid_X_arr.shape, exp_valid_X_arr.shape))
    print("chem={:} | dg={:} | exp={:} ".format(
           chem_test_X_arr.shape, dg_test_X_arr.shape, exp_test_X_arr.shape))
    if args.normalization_str == "tanh_norm":
        ...
        chem_valid_X_arr, mmean1, sstd1, mmean2, sstd2, feat_filtt = PathComb.normalize(chem_valid_X_arr, 
                                                      mean1, std1, mean2, std2, feat_filt=feat_filt, norm="tanh_norm")
        chem_test_X_arr, mean1, std1, mean2, std2, feat_filt = PathComb.normalize(chem_test_X_arr, 
                                                      mean1, std1, mean2, std2, feat_filt=feat_filt, norm="tanh_norm")
        #dg
        dg_train_X_arr, mean1, std1, mean2, std2, feat_filt = PathComb.normalize(dg_train_X_arr, norm="tanh_norm")
        dg_valid_X_arr, mmean1, sstd1, mmean2, sstd2, feat_filtt = PathComb.normalize(dg_valid_X_arr, 
                                                      mean1, std1, mean2, std2, feat_filt=feat_filt, norm="tanh_norm")
        dg_test_X_arr, mean1, std1, mean2, std2, feat_filt = PathComb.normalize(dg_test_X_arr, 
                                                      mean1, std1, mean2, std2, feat_filt=feat_filt, norm="tanh_norm")
        # exp
        exp_train_X_arr, mean1, std1, mean2, std2, feat_filt = PathComb.normalize(exp_train_X_arr, norm="tanh_norm")
        exp_valid_X_arr, mmean1, sstd1, mmean2, sstd2, feat_filtt = PathComb.normalize(exp_valid_X_arr,
                                                      mean1, std1, mean2, std2, feat_filt=feat_filt, norm="tanh_norm")
        exp_test_X_arr, mean1, std1, mean2, std2, feat_filt = PathComb.normalize(exp_test_X_arr, 
                                                      mean1, std1, mean2, std2, feat_filt=feat_filt, norm="tanh_norm")

    elif args.normalization_str == "tanh":
        # chem
        chem_train_X_arr, mean1, std1, feat_filt = PathComb.normalize(chem_train_X_arr, norm="tanh")
        chem_valid_X_arr, mmean1, sstd1, feat_filtt = PathComb.normalize(chem_valid_X_arr, 
                                                      mean1, std1, feat_filt=feat_filt, norm="tanh")
        chem_test_X_arr, mean1, std1, feat_filt = PathComb.normalize(chem_test_X_arr, 
                                                      mean1, std1, feat_filt=feat_filt, norm="tanh")
        # chem
        dg_train_X_arr, mean1, std1, feat_filt = PathComb.normalize(dg_train_X_arr, norm="tanh")
        dg_valid_X_arr, mmean1, sstd1, feat_filtt = PathComb.normalize(dg_valid_X_arr, 
                                                      mean1, std1, feat_filt=feat_filt, norm="tanh")
        dg_test_X_arr, mean1, std1, feat_filt = PathComb.normalize(dg_test_X_arr, 
                                                      mean1, std1, feat_filt=feat_filt, norm="tanh")
        # exp
        exp_train_X_arr, mean1, std1, feat_filt = PathComb.normalize(exp_train_X_arr, norm="tanh")
        exp_valid_X_arr, mmean1, sstd1, feat_filtt = PathComb.normalize(exp_valid_X_arr, 
                                                      mean1, std1, feat_filt=feat_filt, norm="tanh")
        exp_test_X_arr, mean1, std1, feat_filt = PathComb.normalize(exp_test_X_arr, 
                                                      mean1, std1, feat_filt=feat_filt, norm="tanh")
        ...
        chem_valid_X_arr, mmean1, sstd1, feat_filtt = PathComb.normalize(chem_valid_X_arr, 
                                                      mean1, std1, feat_filt=feat_filt, norm="norm")
        chem_test_X_arr, mean1, std1, feat_filt = PathComb.normalize(chem_test_X_arr, 
                                                      mean1, std1, feat_filt=feat_filt, norm="norm")
        # chem
        ...
        chem_train_X_arr = scaler.fit_transform(chem_train_X_arr)
        chem_valid_X_arr = scaler.transform(chem_valid_X_arr)
        chem_test_X_arr = scaler.transform(chem_test_X_arr)
        # dg
        scaler = skpre.StandardScaler()
    ...
    dropout_float = 0.5
    model = PathComb.generate_network_CHEM_CHEM_DG_DG_EXP(n_chem, n_dg, n_exp, inDrop_float, dropout_float)
    print(model.summary())
    ...
    if args.train_test_mode == 1:
        ...
        if args.weightedLoss_bool == True:
            ...
            print("    Loss={:}".format("weighted MSE"))
            min_s = np.amin(train_df['resp'].values)
            loss_weight = np.log(train_df['resp'].values - min_s + np.e)
            model = PathComb.chem_dg_exp_trainer(model, l_rate, chem_train_X_arr, dg_train_X_arr, exp_train_X_arr, train_y_arr, 
                                                 chem_valid_X_arr, dg_valid_X_arr, exp_valid_X_arr, valid_y_arr, max_epoch, batch_size,
    ...
    prediction_arr = PathComb.predict(model, [chem_test_X_arr, dg_test_X_arr, exp_test_X_arr])
    ...
    if args.shap_bool == True:
        ...
        bg_chem = chem_train_X_arr[:100]
        bg_dg = dg_train_X_arr[:100]
        bg_exp = exp_train_X_arr[:100]

        background = [bg_chem, bg_dg, bg_exp]
        explainer = sp.DeepExplainer(model, background)
        shap_list_list = explainer.shap_values([chem_test_X_arr, dg_test_X_arr, exp_test_X_arr])
        chem_shap_arr, dg_shap_arr, exp_shap_arr = shap_list_list[0]
        shap_arr = np.concatenate( (chem_shap_arr, dg_shap_arr, exp_shap_arr ), axis=1)
        shap_df = pd.DataFrame(shap_arr, index=test_df.index, columns=test_df[chem_col_list+dg_col_list+exp_col_list].columns)
