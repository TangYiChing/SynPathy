...
def generate_network_CHEM_CHEM_DGNet_DGNet_EXP_MUT_CNV(n_chem, n_dg, n_exp, n_mut, n_cnv, inDrop_float, dropout_float):
    """
    return mdoel
    """
    ###
    ...
    # combined: drug-drug-cell
    ###
    dg_exp_mut_cnv_input = Concatenate()([chem_input, dg_input, exp_input, mut_input, cnv_input])
    ...
    # return model
    model = Model(inputs=[chem_input, dg_input, exp_input, mut_input, cnv_input], outputs=dg_exp_mut_cnv_output)
    return model
...
def chem_dg_exp_mut_cnv_trainer(model, l_rate, chem_train_X, dg_train_X, exp_train_X, mut_train_X, cnv_train_X, train_y, chem_val_X, dg_val_X, exp_val_X, mut_val_X, cnv_val_X, val_y, epo, batch_size, earlyStop, modelName, weights=None):
    """
    return trained model
    """
    cb_check = ModelCheckpoint((modelName), verbose=1, monitor='val_loss',save_best_only=True, mode='auto')
    ...
    if weights is not None:
        model.fit([chem_train_X, dg_train_X, exp_train_X, mut_train_X, cnv_train_X], train_y, epochs=epo, shuffle=True, batch_size=batch_size,verbose=1,
                  validation_data=([chem_val_X, dg_val_X, exp_val_X, mut_val_X, cnv_val_X], val_y), sample_weight=weights,
                  callbacks=[EarlyStopping(monitor='val_loss', mode='auto', patience = earlyStop),cb_check])
    else:
        model.fit([chem_train_X, dg_train_X, exp_train_X, mut_train_X, cnv_train_X], train_y, epochs=epo, shuffle=True, batch_size=batch_size,verbose=1,
                  validation_data=([chem_val_X, dg_val_X, exp_val_X, mut_val_X, cnv_val_X], val_y),
                  callbacks=[EarlyStopping(monitor='val_loss', mode='auto', patience = earlyStop),cb_check])
    return model
